<html>
	<head>
	<title>Perpustakaan</title>
	<link href="style.css" rel="stylesheet" type="text/css" media="all" />
	</head>
		<body align = "center">
		<div class="atas">
		<img  src="20540172.png" width="100" height="100" align="left">
		<h2><center>SISTEM INFORMASI PERPUSTAKAAN SMA PGRI SRONO</h2></center>
		
		<center><h3>Jl. MOJOPAHIT NO:03 SRONO</h3></center>
		</div>
		<div class="tengah">
            <nav>
                <ul>
            <li align="left"><a href="tampilanadmin.php" >Beranda</a></li>
            <li align="left"><a href="index.php" onClick="return confirm ('Yakin?')">Logout</a></li>
                    </ul>
                </nav>
		</div>
		<div class="home">
		<div class="background"></div>

		
<table width="270" border="4" align="center">
	<tr>
	<td> No </td>
	<td> Nama </td>
	<td> Kelas </td>
	</tr>
	<tr>
	<td> 1</td>
	<td> Adi W </td>
	<td> X4 </td>
	</tr>
	<tr>
	<td> 2 </td>
	<td> Nia Sa </td>
	<td> X4 </td>
	</tr>
	<tr>
	<td> <input type="submit" name="ubahanggota" value="Tambah" align="center"/>
 </td>
	<td> <input type="submit" name="ubahanggota" value="Update" align="center"/>
 </td>
	<td> <input type="submit" name="tambahanggota" value="Delete" align="center"/>
 </td>
	</tr>
</table>

 </body>
 </html>